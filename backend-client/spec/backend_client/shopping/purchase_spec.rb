...
describe BackendClient::Purchase do
  ...
  describe '.instantiate' do
    ...
    it 'sets payment_method correctly' do
      expect(model.payment_method).to be_an_instance_of(BackendClient::PaymentMethod)
    end

    it 'sets billing_address correctly' do
      expect(model.billing_address).to be_an_instance_of(BackendClient::Address)
    end
    ...
    it 'sets payment correctly' do
      expect(model.payment).to be_an_instance_of(BackendClient::Transaction)
    end

    it 'sets orders correctly' do
      expect(model.orders).to contain_exactly(
        an_instance_of(BackendClient::Order)
      )
    end
  end
  ...
  describe '#return_order' do
    ...
    it 'returns order' do
      ...
      expect(doubled_resource).to receive(:put).with({}).and_return(resource_payload)
      expect do
        expect(model.return_order(order_id)).to be_an_instance_of(BackendClient::Order)
      end.to change { model.attributes }
    end
  end
end
